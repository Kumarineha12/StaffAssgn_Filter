...
import { fetchid, fetchRMs, fetchBuilders, fetchCommunities,fetchtopCommunities } from "./newApiServices";
...
const St = ({ rmfm, setRmfm }) => {
  ...
  useEffect(() => {
    const fetchtopCommunitie = async () => {
      if (!selectedBuilder) {
        setCommunities([]);
        return;
      }
      try {
        const res = await fetchtopCommunities(selectedBuilder);
        settopCommunities(res.data.result);
      } catch (error) {
  ...
  // Calculate filtered communities based on selected builder
  // const filteredCommunities = selectedBuilder 
  //   ? communities.filter(c => c.builder_id === parseInt(selectedBuilder))
  //   : communities;

  // Calculate filtered indices and pagination
  ...
  //     //   return (
  //     //     record.community_id === parseInt(selectedCommunity) &&
  //     //     communities.some(c => c.id === record.community_id && c.builder_id === parseInt(selectedBuilder))
  //     //   );
  //     // }
  //     if (selectedBuilder) {
  ...
  //     //   return record.community_id === parseInt(selectedCommunity);
  //     // }
  //     return true;
  //   });
  ...
  const filteredIndices = rmfm
    ...
    .filter(index => {
      ...
      const matchesBuilder = selectedBuilder
        ? communities.some(c => c.id === record.community_id && c.builder_id === parseInt(selectedBuilder))
        : true;
      ...
      return matchesBuilder && matchesCommunity;
    });
...
//     newRmfm[index].community_id = communityId;
    
//     // Use the same filteredCommunities that populate the dropdown
//     const selectedCommunity = filteredCommunities.find(c => c.id === parseInt(communityId));
//     if (selectedCommunity) {
//       newRmfm[index].community_name = selectedCommunity.name;
//     }
